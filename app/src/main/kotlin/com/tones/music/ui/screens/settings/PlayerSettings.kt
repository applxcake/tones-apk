...
import com.tones.music.ui.component.SliderPreference
...
fun PlayerSettings(
    ...
    val (smoothTransitionEnabled, onSmoothTransitionEnabledChange) = rememberPreference(
        SmoothTransitionEnabledKey,
        defaultValue = false
    )
    val (smoothTransitionDuration, onSmoothTransitionDurationChange) = rememberPreference(
        SmoothTransitionDurationKey,
        defaultValue = 3
    )
    ...
    Column(
        ...
        SwitchPreference(
            title = { Text("Smooth Transition") },
            description = "Smooth transition between songs",
            icon = { Icon(painterResource(R.drawable.volume_up), null) },
            checked = smoothTransitionEnabled,
            onCheckedChange = onSmoothTransitionEnabledChange
        )

        if (smoothTransitionEnabled) {
            SliderPreference(
                title = { Text("Smooth Transition Duration (seconds)") },
                icon = { Icon(painterResource(R.drawable.volume_up), null) },
                value = smoothTransitionDuration.toFloat(),
                onValueChange = { onSmoothTransitionDurationChange(it.toInt()) },
                dialogTitle = "Smooth Transition Duration",
                valueRange = 1f..12f,
                resetValue = 3f
            )
        }
