        ...
        YouTube.playlist("LM").completed().onSuccess { page ->
            ...
            remoteSongs.forEachIndexed { index, song ->
                launch {
                    ...
                    if (dbSong == null) {
                        database.insert(song.toMediaMetadata()) { songEntity ->
                            songEntity.copy(liked = true, likedDate = timestamp)
                        }
                    } else {
                        // Always ensure artist relationships are properly established
                        // and update the song's liked status
                        database.update(dbSong, song.toMediaMetadata())
            ...
            remoteSongs.forEach { song ->
                launch {
                    ...
                    if (dbSong == null) {
                        ...
                        // Always ensure artist relationships are properly established
                        // and update the song's library status if needed
                        database.update(dbSong, song.toMediaMetadata())
